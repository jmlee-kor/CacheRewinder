...
if [[ "$ARGC" != 1 ]]; then # Bad number of arguments.
   ...
   echo ""
    ...
    echo ""
    echo "USAGE: run_gem5_x86_spec06_benchmark.sh <BENCHMARK>"
    echo "EXAMPLE: ./run_gem5_x86_spec06_benchmark.sh bzip2"
    echo ""
...
MAX_INSTS=50000000                      # Number of instructions to be simulated
CHECKPOINT_CONFIG="ooo_8Gmem_10B"    # Name of directory inside CKPT_PATH
...
# Output Dir
OUTPUT_DIR=$GEM5_PATH/output/SPEC-$BENCHMARK
...
RUN_DIR=$SPEC_PATH/benchspec/CPU2006/$BENCHMARK_CODE/run/run_base_ref_gcc44-64bit.0000
...
$GEM5_PATH/build/X86_MESI_Two_Level/gem5.opt \
    --outdir=$OUTPUT_DIR \
    $GEM5_PATH/configs/example/spec06.py \
    --benchmark=$BENCHMARK \
    ...
    --checkpoint-restore=$INST_TAKE_CHECKPOINT \
    --maxinsts=$MAX_INSTS \
    --at-instruction \
    --l1i_assoc=8 \
    --l1d_assoc=8 \
